...
class DataTransformer(Routine):
    ...
    def __init__(self):
        ...
        # Define output event
        self.output_event = self.define_event("output", ["transformed_data", "transformation_applied", "errors"])
    ...
    def _register_builtin_transformations(self):
        ...
        builtins = {
            ...
            "remove_none": lambda x: {k: v for k, v in x.items() if v is not None} if isinstance(x, dict) else x,
        }
        ...
        for transform in transformations:
            try:
                if isinstance(transform, str):
                    ...
                    applied_transformations.append(transform.__name__ if hasattr(transform, "__name__") else "custom")
                else:
                    errors.append(f"Invalid transformation: {transform}")
            except Exception as e:
                errors.append(f"Error applying {transform}: {str(e)}")
                self._track_operation("transformation_errors", success=False, transform=str(transform), error=str(e))
