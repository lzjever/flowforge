...
class JobSubmitRequest(BaseModel):
    ...
    flow_id: str = Field(..., description="Flow ID (used to find or create Worker)")
    worker_id: Optional[str] = Field(
        None, description="Target Worker ID. If None, creates new Worker."
    )
    routine_id: str = Field(..., description="Target routine to receive data")
    slot_name: str = Field(..., description="Target slot on the routine")
    data: Dict[str, Any] = Field(default_factory=dict, description="Data to send to the slot")
    job_id: Optional[str] = Field(
        None, description="Optional custom job ID. Auto-generated if not provided."
    )
    metadata: Optional[Dict[str, Any]] = Field(
        None, description="Job metadata (user_id, source, trace_id, etc.)"
    )
    idempotency_key: Optional[str] = Field(
        None, description="Idempotency key to prevent duplicate submissions"
    )
...
class JobResponse(BaseModel):
    ...
    worker_id: str = Field(..., description="Worker processing this Job")
    flow_id: str = Field(..., description="Flow being executed")
...
class JobListResponse(BaseModel):
    ...
    limit: int = Field(100, description="Maximum jobs per page", ge=1, le=1000)
    offset: int = Field(0, description="Number of jobs skipped", ge=0)
...
class JobOutputResponse(BaseModel):
    ...
    is_complete: bool = Field(..., description="Whether job has completed")
...
class JobTraceResponse(BaseModel):
    ...
    job_id: str = Field(..., description="Job identifier")
    trace_log: List[Dict[str, Any]] = Field(..., description="Execution trace entries")
    total_entries: int = Field(..., description="Total number of trace entries")
...
class JobStartRequest(BaseModel):
    ...
    flow_id: str = Field(..., description="Flow ID to execute (DEPRECATED: use JobSubmitRequest)")
    runtime_id: Optional[str] = Field(None, description="Runtime ID (DEPRECATED)")
...
class PostToJobRequest(BaseModel):
    ...
    routine_id: str = Field(..., description="Routine ID (DEPRECATED: use JobSubmitRequest)")
    slot_name: str = Field(..., description="Slot name (DEPRECATED: use JobSubmitRequest)")
    data: Optional[Dict[str, Any]] = Field(
        None, description="Data to send (DEPRECATED: use JobSubmitRequest)"
    )
