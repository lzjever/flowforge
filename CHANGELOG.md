...
- Server startup: Deduplicate routine directories by resolving to absolute paths to avoid "already registered" error

## [0.15.2] - 2026-02-14

### Fixed

- Install script: Use `--upgrade --force` for uv to bypass cache issue and ensure latest version is installed

## [0.15.1] - 2026-02-14

### Fixed

- Code quality improvements in server startup (avoid redundant factory reassignment)
- Lint fixes in benchmark tests
...
- **CLI init command**: Now accepts positional argument (`routilux init my-project`) instead of requiring `--name` option

### Tests

- Added comprehensive CLI integration tests to prevent basic command errors
- Tests now cover: init, help, version, list, run, server, validate commands

## [0.14.3] - 2026-02-13

### Added

- **Server dependencies**: Added `[server]` and `[all]` extras with uvicorn, fastapi, websockets, slowapi

### Changed

- **Installer**: Now installs with `[all]` extras by default (includes CLI + server dependencies)

## [0.14.2] - 2026-02-13

### Changed

- **Installer auto-upgrade**: install script now automatically upgrades if routilux is already installed
