...
- **Unified Execution Context Management**: Introduced `ExecutionContext` as a single source of truth for all execution context information (flow, worker_state, routine_id, job_context)
  - Added `_current_execution_context` ContextVar in `context.py`
  - Added `get_current_execution_context()` and `set_current_execution_context()` functions
  - `ExecutionContext` moved from `routine.py` to `context.py` for better organization

- **Enhanced JobContext API**: Added methods for routine-specific data management
  - `set_routine_data(routine_id, key, value)`: Set routine-specific data with automatic namespacing
  - `get_routine_data(routine_id, key, default)`: Get routine-specific data with automatic namespacing
  - Updated `set_data()` and `get_data()` documentation to clarify usage for shared data

- **Routine Convenience Methods**: Added methods to `Routine` class for easier job data management
  - `set_job_data(key, value)`: Set job-level data for this routine (automatically namespaced by routine_id)
  - `get_job_data(key, default)`: Get job-level data for this routine (automatically namespaced by routine_id)
  - These methods automatically handle routine_id namespacing to prevent key collisions

### Changed

- **Architectural Refactoring**: Unified execution context management
  - `Routine.get_execution_context()` now directly retrieves from ContextVar instead of assembling from multiple sources
  - `WorkerExecutor._execute_task()` now sets complete `ExecutionContext` in a single ContextVar
  - Removed dependency on `routine._current_flow` and `routine._current_runtime` instance variables
  - All execution context information is now provided via `ExecutionContext` ContextVar

- **State Management Improvements**:
  - `Routine.get_state()`, `set_state()`, and `update_state()` now use `ExecutionContext` instead of `_current_flow`
  - `Routine.emit()` now retrieves runtime from `worker_state._runtime` instead of `_current_runtime`
  - `Routine.set_config()` now checks execution context via `ExecutionContext` instead of `_current_flow`

- **Error Handling**: Improved error messages and validation
  - `LoopControllerRoutine` and `CounterRoutine` now raise `RuntimeError` when required context is missing (removed fallback logic)
  - Better error messages indicating what context is required

### Fixed

- **State Isolation**: Fixed state key collision issues in `job.data`
  - `LoopControllerRoutine` now uses `routine_id` namespacing for iteration count (`loop_iteration_{routine_id}`)
  - Prevents state collisions when multiple instances of the same routine type exist in a flow

- **Context Variable Management**: Fixed duplicate ContextVar definitions
  - Removed duplicate `_current_worker_state` definition in `routine.py`
  - All context variables now managed in `context.py`

- **Thread Safety**: Removed instance variable dependencies that could cause issues in multi-threaded environments
  - Removed `routine._current_flow` instance variable
  - Removed `routine._current_runtime` instance variable
  - All context now provided via thread-safe ContextVars

### Removed

- **Deprecated Instance Variables**: Removed thread-unsafe instance variables
  - `routine._current_flow`: Replaced by `ExecutionContext.flow`
  - `routine._current_runtime`: Replaced by `worker_state._runtime` access

- **Redundant Functions**: Removed duplicate convenience functions from `routine.py`
  - `get_current_worker_state()`: Now only in `context.py`
  - `set_current_worker_state()`: Now only in `context.py`

### Performance

- **Reduced Lookups**: Direct access to `ExecutionContext` eliminates multiple lookups and assembly operations
- **Simplified Access**: `get_execution_context()` is now a simple ContextVar get instead of complex assembly logic

### Testing

- Updated all tests to use new `ExecutionContext` setup pattern
- Added comprehensive tests for `set_job_data()` and `get_job_data()` methods
- Added tests for `set_routine_data()` and `get_routine_data()` methods
- Verified state isolation between multiple routine instances
