...
class RetryRouter(Routine):
    ...
    def __init__(self, max_retries: int = 3):
        ...
        self.final_success_event = self.define_event("final_success", ["result", "data", "total_attempts"])
        self.final_failure_event = self.define_event("final_failure", ["error", "data", "total_attempts"])
    
    def _handle_input(self, result: Any = None, error: str = None, data: Any = None, retry_count: int = 0, **kwargs):
        ...
        if result and not error:
            ...
            print(f"  [RetryRouter] Success after {total_attempts} attempts, forwarding to downstream")
            self.emit("final_success",
                result=result,
                data=data,
            ...
            if retry_count < max_retries:
                ...
                # 超过最大重试次数：发送最终失败
                total_attempts = retry_count + 1
                print(f"  [RetryRouter] Max retries ({max_retries}) exceeded, sending final failure")
                self.emit("final_failure",
                    error=error,
                    data=data,
                    total_attempts=total_attempts
                )
