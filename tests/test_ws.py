...
def collect_events(
    ws, event_type=None, max_messages=20, timeout_per_message=0.3, max_wait_time=None
):
    ...
    actual_timeout = (
        min(timeout_per_message, 0.2)
        if max_wait_time is None
        else min(timeout_per_message, 0.2, max_wait_time / max_messages)
    )

    try:
        for message in receive_messages_with_timeout(
            ws, max_messages=max_messages, timeout_per_message=actual_timeout
        ):
            message_count += 1
...
def simple_flow():
    ...
    class SimpleRoutine(Routine):
        ...
        def logic(self, slot_data, policy_message, worker_state):
            ...
            value = (
                slot_data.get("input", {}).get("value", 0)
                if isinstance(slot_data, dict)
                else (slot_data.get("value", 0) if hasattr(slot_data, "get") else 0)
            )
...
class TestEventFormat:
    ...
    def test_event_has_required_fields(self, client, simple_flow):
        ...
        with client.websocket_connect("/api/v1/websocket") as ws:
            ...
            events_received = collect_events(
    ...
    def test_job_completed_event(self, client, flow_with_completion):
        ...
        if not job_completed:
            ...
            if final_response.status_code == 200:
                ...
                pytest.fail(
                    f"Job did not complete within {max_wait}s. Final status: {final_data.get('status')}, started_at: {final_data.get('started_at')}, completed_at: {final_data.get('completed_at')}"
                )
            else:
                pytest.fail(
                    f"Job did not complete within {max_wait}s. Last status: {last_status}, Job not found in API"
                )
        ...
        with client.websocket_connect("/api/v1/websocket") as ws:
            ...
            if len(routine_started_events) == 0:
                ...
                if len(job_started) > 0:
                    ...
                    pytest.skip(
                        "routine_started events not received (may not be implemented for all cases)"
                    )

            assert len(routine_started_events) > 0, (
                f"No routine_started events received. All events: {[e.get('type') for e in all_events]}"
            )
    ...
    def test_routine_completed_event(self, client, multi_routine_flow):
        ...
        with client.websocket_connect("/api/v1/websocket") as ws:
            ...
            for _ in range(10):  # Try up to 10 times
                try:
                    msg_list = list(
                        receive_messages_with_timeout(ws, max_messages=3, timeout_per_message=0.1)
                    )
                    for msg in msg_list:
                ...
                routine_completed_events = [
                    e for e in all_events if e.get("type") == "routine_completed"
                ]
            ...
            if len(routine_completed_events) == 0:
                ...
                if len(routine_started) > 0:
                    pytest.skip(
                        "routine_completed events not received (may not be implemented for all cases)"
                    )
                elif len(job_started) > 0:
                    pytest.skip(
                        "No routine events received (routines may not execute in this flow)"
                    )
                else:
                    ...
                    if job_response.status_code == 200:
                        ...
                        pytest.fail(
                            f"No events received and job not found. All events: {[e.get('type') for e in all_events]}"
                        )

            assert len(routine_completed_events) > 0, (
                f"No routine_completed events received. All events: {[e.get('type') for e in all_events]}"
            )
        ...
        try:
            ...
            with client.websocket_connect("/api/v1/websocket") as ws:
                ...
                if len(events) > 0:
                    ...
                    if len(routine_started) > 0:
                        pytest.skip(
                            "routine_failed event not received (may not be implemented for all cases)"
                        )
                    else:
                        pytest.skip(
                            "No routine events received (routines may not execute in this flow)"
...
class TestSlotAndEventEvents:
    ...
    def test_slot_called_event(self, client, multi_routine_flow):
        ...
        with client.websocket_connect("/api/v1/websocket") as ws:
            ...
            slot_called_events = collect_events(
                ws,
                event_type="slot_called",
                max_messages=20,
                timeout_per_message=1.0,
                max_wait_time=10.0,
            )
    ...
    def test_event_emitted_event(self, client, multi_routine_flow):
        ...
        with client.websocket_connect("/api/v1/websocket") as ws:
            ...
            event_emitted_events = collect_events(
                ws,
    ...
    def test_breakpoint_hit_event(self, client, simple_flow):
        ...
        with client.websocket_connect("/api/v1/websocket") as ws:
            ...
            if len(events) == 0:
                ...
                if len(job_started) > 0:
                    pytest.skip(
                        "breakpoint_hit event not received (breakpoint may not have been hit)"
                    )

            assert len(events) > 0, (
                f"breakpoint_hit event not received. All events: {[e.get('type') for e in all_events]}"
            )
            message = events[0]
            assert message["job_id"] == job_id
            assert "timestamp" in message
...
class TestEdgeCases:
        ...
        with client.websocket_connect("/api/v1/websocket") as ws:
            ...
            for _ in range(5):
                ...
                if message2.get("type") == "subscribed" and message2.get("job_id") == job_id:
                    ...
                    message2.get("type") == "already_subscribed"
                    and message2.get("job_id") == job_id
                ):
                    subscribed2 = True
                    break
    ...
    def test_invalid_message_type(self, client):
        ...
        with client.websocket_connect("/api/v1/websocket") as ws:
            ...
            try:
                ...
                messages = list(
                    collect_events(ws, max_messages=1, timeout_per_message=0.5, max_wait_time=1.0)
                )
                if messages:
                    message = messages[0]
    ...
    def test_empty_event_data(self, client, simple_flow):
        ...
        with client.websocket_connect("/api/v1/websocket") as ws:
            ...
            for event in events_received:
                assert isinstance(event["data"], dict), f"Event data must be dict: {event}"
...
class TestConcurrentSubscriptions:
    ...
    def test_multiple_jobs_concurrent_events(self, client, simple_flow):
        ...
        with client.websocket_connect("/api/v1/websocket") as ws:
            ...
            assert len(jobs_with_events) > 0, (
                f"No events received for any job. All events: {[e.get('type') for e in all_events]}"
            )
            ...
            jobs_with_started = [
                job_id
                for job_id in job_ids
                if "job_started" in [e["type"] for e in events_by_job[job_id]]
            ]
