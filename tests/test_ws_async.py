...
async def collect_events_async(
    ...
    while message_count < max_messages:
        ...
        try:
            message = await asyncio.wait_for(ws.receive_json(), timeout=timeout_per_message)
            message_count += 1
...
class TestJobLifecycleEvents:
    ...
    async def test_job_completed_event(self, async_client, flow_with_completion):
        ...
        async with aconnect_ws("http://test/api/v1/websocket", async_client) as ws:
            ...
            if len(events) == 0:
                pytest.skip(
                    "Job completed successfully but job_completed event not received via WebSocket (event may be published asynchronously)"
                )
    ...
    async def test_job_failed_event(self, async_client, flow_with_failure):
        ...
        async with aconnect_ws("http://test/api/v1/websocket", async_client) as ws:
            ...
            if len(events) == 0:
                pytest.skip(
                    "Job failed successfully but job_failed event not received via WebSocket (event may be published asynchronously)"
                )
