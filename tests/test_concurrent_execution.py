...
class TestConcurrentRoutineExecution:
    ...
    def test_single_event_multiple_slots_concurrent(self):
        ...
        # 验证所有 routines 都执行了
        assert len(execution_order) == 3, f"Expected 3 routines to execute, got {len(execution_order)}"
    ...
    def test_sequential_vs_concurrent_performance(self):
        ...
        assert concurrent_time < 0.4, \
            f"并发执行时间 {concurrent_time:.3f}s 应该小于 0.4 秒"
...
class TestConcurrentErrorHandling:
    ...
    def test_concurrent_error_continue_strategy(self):
        ...
        # 验证：即使有错误，其他消息也应该被处理
        assert len(results) >= 4, f"Expected at least 4 results, got {len(results)}"  # 至少处理了 4 个（除了失败的）
        assert job_state.status == "completed"
...
class TestConcurrentIntegration:
    ...
    def test_complex_concurrent_flow(self):
        ...
        assert len(results) == 3, f"Expected 3 worker results, got {len(results)}"  # 三个 worker 都应该执行
        assert len(aggregator.final_result) == 3, f"Expected 3 aggregated results, got {len(aggregator.final_result)}"  # 聚合器应该收到所有结果
        assert job_state.status == "completed"
