...
- 修复 `routilux/__init__.py` 的 import 块排序
- 状态: ✅ 完成

**L-2**: 添加 types-PyYAML>=6.0.0 到 dev 依赖
- 消除 mypy import-untyped 错误
- 状态: ✅ 完成

**L-3**: 类型注解改进
- 添加 `-> None` 到 6 个 `__init__` 方法
- 添加 `check_untyped_defs=true` 到 mypy 配置
- **注意**: 完整严格类型检查 (`disallow_untyped_defs=true`) 仍需约 2 周工作量，留待后续迭代

**状态**: 准备发布 ✅

所有 v01 检查清单中的轻微问题已解决:
- L-1, L-2: 完全解决
- L-3: 部分解决 (关键 __init__ 方法已添加返回类型注解)
