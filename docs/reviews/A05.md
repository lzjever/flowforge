...
- Reduced TYPE_CHECKING usage in flow.py by 60% (5 → 2 imports)
- Refactored routine.emit() to use ExecutionContext
- Event.py now uses IEventHandler protocol instead of Runtime
- Runtime.py explicitly implements IEventHandler

### Results
- Before: 15 files with TYPE_CHECKING
- After: 15 files but with significant reduction in critical paths
- All core tests passing (9/9)
- No import errors
- Protocol-based architecture established

### Commits
- 56a9d78: test(core): add Protocol interface tests
- 5ffc672: refactor(core): routine.emit() uses ExecutionContext for routing
- aa2edc84: fix(context): remove duplicate ContextVar definition
- fbdb208: refactor(core): direct import Event, Slot, Connection in flow.py
- 0deb429: refactor(core): use IEventHandler protocol in event.py
- 93f1a16: refactor(core): Runtime explicitly implements IEventHandler
